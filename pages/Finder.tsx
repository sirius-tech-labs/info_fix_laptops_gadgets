  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-md hover:shadow-lg transition-all flex flex-col">
      ...
      <div className="relative h-36 bg-gray-50 flex items-center justify-center overflow-hidden">
        ...
        <span className="absolute top-2 left-2 bg-tech-blue/90 text-white text-[10px] font-black px-2 py-0.5 rounded-full uppercase">
          {laptop.condition}
        </span>
      </div>
      ...
      <div className="p-3 flex flex-col flex-grow">
        ...
        <div className="mt-auto">
          <p className="text-tech-blue font-black text-base">{formatPrice(laptop.price)}</p>
          {discount > 0 && (
      ...
      <div className="px-3 pb-3 flex gap-2">
        ...
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl font-black text-xs border-2 border-gray-200 text-gray-700 hover:border-tech-blue hover:text-tech-blue transition-all active:scale-95"
        >
...
const Finder: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'bot',
      text: 'Hello! 👋 I\'m your GIANT EDGE TECHNOLOGIES Smart Assistant. I can help you find the perfect laptop based on your budget and needs — with nationwide delivery across Nigeria! What can I find for you today?',
    }
  ]);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100 flex flex-col" style={{ height: 'calc(100vh - 160px)', minHeight: '560px' }}>
        ...
        {/* Header */}
        <div className="bg-gradient-to-r from-tech-blue to-blue-700 p-5 text-white flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <p className="text-gray-400 font-bold">GIANT EDGE TECHNOLOGIES | Ondo state, Nigeria</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
            {QUICK_PROMPTS.map((prompt, i) => (
              <button
                ...
                disabled={isLoading}
                className="flex-shrink-0 text-[11px] font-bold bg-white border border-gray-200 text-gray-600 px-3 py-1.5 rounded-full hover:border-tech-blue hover:text-tech-blue transition-colors whitespace-nowrap disabled:opacity-50"
              >
                {prompt}
              </button>
            ))}
        ...
        <div className="bg-white p-4 border-t border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-grow px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-tech-blue focus:bg-white transition-all text-sm text-gray-900"
            />
            <button
              onClick={handleSend}
              disabled={isLoading || !input.trim()}
              className="flex-shrink-0 bg-tech-blue text-white p-3 rounded-xl hover:bg-blue-900 transition-colors disabled:opacity-40 active:scale-95"
            >
              <Send size={20} />
            </button>
          </div>
          <p className="text-[10px] text-gray-400 mt-2 text-center uppercase tracking-widest font-bold flex items-center justify-center gap-1">
            <Zap size={10} className="text-tech-blue" /> Smart Matching · Forever Free
          </p>
        </div>
      </div>
    </div>
