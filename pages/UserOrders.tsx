...
const statusConfig: Record<string, { icon: React.ElementType; color: string; bg: string; label: string }> = {
    ...
    [OrderStatus.IN_PROGRESS]: { icon: Clock, color: 'text-orange-500', bg: 'bg-orange-50', label: 'In Progress' },
    ...
    useEffect(() => {
        ...
        const fetchMyOrders = async () => {
            ...
            try {
                ...
                try {
                    const saved = JSON.parse(localStorage.getItem('infofix_orders') || '[]');
    ...
    if (authLoading || isLoading) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
                <Loader size={40} className="text-tech-blue animate-spin" />
            ...
            <div className="min-h-[60vh] flex items-center justify-center px-4">
                <div className="bg-white rounded-[2.5rem] shadow-2xl p-12 max-w-md w-full text-center border border-gray-100">
                    <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-6">
                        <Package size={40} className="text-tech-blue" />
                    </div>
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-3 bg-tech-blue text-white font-black py-4 rounded-2xl hover:bg-blue-900 transition-all shadow-lg active:scale-95"
                    >
                        <LogIn size={20} />
                        Sign in to View Orders
                    </button>
                    <Link to="/shop" className="block mt-4 text-gray-400 text-sm font-bold hover:text-tech-blue transition">
                        Continue Shopping
                    </Link>
                </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-12">
            ...
            {orders.length === 0 ? (
                <div className="bg-white p-16 rounded-[3rem] text-center border-2 border-dashed border-gray-200">
                    ...
                    <Link
                        to="/shop"
                        className="inline-flex items-center gap-2 bg-tech-blue text-white px-8 py-3 rounded-xl font-black hover:bg-blue-900 transition shadow-lg"
                    >
                                ...
                                {isExpanded && (
                                    <div className="border-t border-gray-100 px-6 pb-6 pt-4 space-y-4">
                                        ...
                                        <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                                            ...
                                            <span className="text-xl font-black text-tech-blue">{formatPrice(order.totalAmount)}</span>
                                        </div>
                                    </div>
                                )}
                            </div>
