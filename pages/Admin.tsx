...
const Admin: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        const saved = localStorage.getItem('wonderful_autos_orders');
  ...
  useEffect(() => {
    if (orders.length > 0) {
      localStorage.setItem('wonderful_autos_orders', JSON.stringify(orders));
    }
  }, [orders]);
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-white border-b border-gray-200 py-8 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="flex items-center gap-2 text-tech-blue mb-1">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="bg-gray-100 p-1 rounded-xl flex gap-1 w-full md:w-auto">
              <button
                ...
                className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg font-bold text-sm transition-all ${activeTab === 'inventory' ? 'bg-white text-tech-blue shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Inventory
              </button>
              <button
                onClick={() => setActiveTab('orders')}
                className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg font-bold text-sm transition-all relative ${activeTab === 'orders' ? 'bg-white text-tech-blue shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Orders
            ...
            {activeTab === 'inventory' && (
              <div className="flex gap-2 w-full md:w-auto">
                <button
                  ...
                  className="flex-1 md:flex-none bg-tech-blue text-white px-4 md:px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg hover:bg-blue-900 transition text-xs md:text-sm"
                >
      ...
      <div className="max-w-7xl mx-auto px-4 mt-10">
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
              <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-5">
                <div className="bg-blue-50 p-4 rounded-2xl text-tech-blue"><LaptopIcon size={32} /></div>
                <div>
              ...
              {(Object.entries(groupedInventory) as [string, Laptop[]][]).map(([category, laptops]) => (
                <div key={category}>
                  <h2 className="text-xl font-black text-gray-900 mb-6 flex items-center gap-3 uppercase tracking-tighter italic">
                    <div className="w-2 h-8 bg-tech-blue rounded-full"></div>
                              ...
                              <td className="px-8 py-6">
                                <div className="relative max-w-[180px]">
                                  ...
                                  <input
                                    ...
                                    className="w-full pl-8 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg font-black text-tech-blue focus:ring-2 focus:ring-tech-blue focus:bg-white outline-none"
                                  />
                                </div>
                              </td>
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                ...
                { id: OrderStatus.IN_PROGRESS, icon: Clock, color: 'text-orange-600', bg: 'bg-orange-50' },
                ...
                <button
                  ...
                  className={`text-left bg-white p-6 rounded-3xl border transition-all hover:shadow-md active:scale-95 ${statusFilter === status.id
                    ? 'border-tech-blue ring-4 ring-tech-blue/5 shadow-lg'
              ...
              {statusFilter !== 'all' && (
                <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                  <p className="text-xs font-black text-gray-400 uppercase tracking-widest">
                    Showing: <span className="text-tech-blue">{statusFilter}</span>
                  </p>
                  <button
                    onClick={() => setStatusFilter('all')}
                    className="text-[10px] font-black text-tech-blue uppercase tracking-widest hover:underline flex items-center gap-1"
                  >
                  ...
                  statusOrders.length > 0 ? (
                    <div key={status}>
                      <h2 className="text-xl font-black text-gray-900 mb-6 flex items-center gap-3 uppercase tracking-tighter italic">
                        <div className={`w-2 h-8 rounded-full ${status === OrderStatus.ORDERS ? 'bg-blue-600' :
                          status === OrderStatus.IN_PROGRESS ? 'bg-orange-600' :
                          ...
                          <div key={order.id} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden flex flex-col md:flex-row">
                            <div className="p-8 flex-grow">
                              <div className="flex justify-between items-start mb-6">
                                <div>
                                  <p className="text-tech-blue font-black text-lg mb-1">{order.id}</p>
                    ...
                    statusFilter !== 'all' && (
                      <div key={status} className="bg-white p-20 rounded-[3rem] text-center border-2 border-dashed border-gray-200">
                        ...
                        <button
                          onClick={() => setStatusFilter('all')}
                          className="mt-6 text-tech-blue font-black uppercase text-xs tracking-widest hover:underline"
                        >
                          Show All Orders
                        </button>
                      </div>
        ...
        {showAddForm && (
          <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-4xl rounded-[2.5rem] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
              <div className="bg-tech-blue p-6 text-white flex justify-between items-center shrink-0">
              ...
              {isUploading && (
                <div className="absolute inset-0 z-[110] bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center">
                  <div className="w-12 h-12 border-4 border-tech-blue border-t-transparent rounded-full animate-spin mb-4"></div>
                  <p className="text-tech-blue font-black uppercase tracking-widest text-xs">Processing Image...</p>
                </div>
              )}
              ...
              <form onSubmit={handleAddLaptop} className="p-8 overflow-y-auto space-y-10">
                ...
                <div>
                  <h3 className="text-lg font-black text-gray-900 mb-6 flex items-center gap-2 border-b pb-2">
                    <Layout size={20} className="text-tech-blue" /> Basic Information
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="md:col-span-2 lg:col-span-2">
                      <label className="block text-xs font-black uppercase text-gray-400 mb-2">Laptop Full Name</label>
                      <input required type="text" placeholder="e.g. HP EliteBook 840 G5" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" onChange={e => setNewLaptop({ ...newLaptop, name: e.target.value })} />
                    </div>
                    <div>
                      <label className="block text-xs font-black uppercase text-gray-400 mb-2">Brand</label>
                      <select className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" onChange={e => setNewLaptop({ ...newLaptop, brand: e.target.value })}>
                    ...
                    <div>
                      ...
                      <input required type="number" placeholder="280000" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" onChange={e => setNewLaptop({ ...newLaptop, price: parseInt(e.target.value) || 0 })} />
                    </div>
                    <div>
                      <label className="block text-xs font-black uppercase text-gray-400 mb-2">Compare At Price (Optional)</label>
                      <input type="number" placeholder="350000" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue font-medium" onChange={e => setNewLaptop({ ...newLaptop, originalPrice: parseInt(e.target.value) || 0 })} />
                    </div>
                    <div>
                      <label className="block text-xs font-black uppercase text-gray-400 mb-2">Category</label>
                      <select className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" onChange={e => setNewLaptop({ ...newLaptop, category: e.target.value as Category })}>
                    ...
                    <div className="md:col-span-2 lg:col-span-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <input type="url" placeholder="https://..." className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue h-32" value={newLaptop.image} onChange={e => setNewLaptop({ ...newLaptop, image: e.target.value })} />
                        </div>
                      </div>
                      {newLaptop.image && (
                        <div className="mt-4 flex items-center gap-4 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                          ...
                          />
                          <div>
                            <p className="text-xs font-black text-tech-blue uppercase">Image Selected</p>
                    ...
                    <div className="md:col-span-2 lg:col-span-3">
                      ...
                      <input type="url" placeholder="https://www.youtube.com/embed/..." className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" value={newLaptop.youtubeUrl} onChange={e => setNewLaptop({ ...newLaptop, youtubeUrl: e.target.value })} />
                    </div>
                    <div className="md:col-span-2 lg:col-span-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <textarea
                            ...
                            className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue h-24"
                            onChange={e => {
                              const currentImages = newLaptop.moreImages || [];
                              const remainingSlots = 5 - currentImages.length;
                ...
                <div>
                  <h3 className="text-lg font-black text-gray-900 mb-6 flex items-center gap-2 border-b pb-2">
                    <Cpu size={20} className="text-tech-blue" /> Technical Specifications
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input required type="text" placeholder="Intel Core i5-8250U 8th Gen" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" value={specFields.processor} onChange={e => setSpecFields({ ...specFields, processor: e.target.value })} />
                    </div>
                    <div>
                      <label className="flex items-center gap-2 text-xs font-black uppercase text-gray-400 mb-2"><Layers size={14} /> RAM</label>
                      <input required type="text" placeholder="8GB DDR4 RAM (Upgradable)" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" value={specFields.ram} onChange={e => setSpecFields({ ...specFields, ram: e.target.value })} />
                    </div>
                    <div>
                      <label className="flex items-center gap-2 text-xs font-black uppercase text-gray-400 mb-2"><Database size={14} /> Storage</label>
                      <input required type="text" placeholder="256GB NVMe SSD" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" value={specFields.storage} onChange={e => setSpecFields({ ...specFields, storage: e.target.value })} />
                    </div>
                    <div>
                      <label className="flex items-center gap-2 text-xs font-black uppercase text-gray-400 mb-2"><Monitor size={14} /> Graphics</label>
                      <input required type="text" placeholder="Intel UHD Graphics 620" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" value={specFields.graphics} onChange={e => setSpecFields({ ...specFields, graphics: e.target.value })} />
                    </div>
                    <div>
                      <label className="flex items-center gap-2 text-xs font-black uppercase text-gray-400 mb-2"><Layout size={14} /> OS</label>
                      <input required type="text" placeholder="Windows 11 Pro" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" value={specFields.os} onChange={e => setSpecFields({ ...specFields, os: e.target.value })} />
                    </div>
                    <div>
                      ...
                      <input required type="text" placeholder="USB-C, HDMI, SD Card Reader" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-tech-blue" value={specFields.ports} onChange={e => setSpecFields({ ...specFields, ports: e.target.value })} />
                    </div>
                  </div>
                </div>

                <div className="md:col-span-2 pt-4">
                  <button type="submit" className="w-full bg-tech-blue text-white py-6 rounded-3xl font-black text-xl hover:bg-blue-900 transition shadow-xl flex items-center justify-center gap-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-20">
          <div className="bg-tech-blue rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl">
          ...
          <div className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-xl flex flex-col justify-center">
            ...
            <button className="w-full py-4 border-2 border-tech-blue text-tech-blue rounded-2xl font-black hover:bg-tech-blue hover:text-white transition uppercase tracking-widest text-xs">
