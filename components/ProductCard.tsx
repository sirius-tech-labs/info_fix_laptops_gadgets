...
const ProductCard: React.FC<ProductCardProps> = ({ laptop }) => {
  ...
  return (
    <>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 transition-all group overflow-hidden flex flex-col h-full relative hover:shadow-xl border-b-4 border-b-transparent hover:border-b-tech-blue">
        {/* Sale Tag */}
        ...
        <div className="relative aspect-[4/3] bg-gray-50 overflow-hidden block">
          <Link to={`/product/${laptop.id}`} className="absolute inset-0">
            {!imageLoaded && (
              <div className="absolute inset-0 bg-gray-50 flex items-center justify-center z-0">
                <Loader2 className="w-6 h-6 text-tech-blue animate-spin opacity-20" />
              </div>
            )}
            <img
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1 pointer-events-none">
            <span className="bg-white/90 backdrop-blur-sm text-tech-blue text-[9px] uppercase font-black px-2 py-1 rounded shadow-sm">
              {laptop.condition}
            </span>
            {laptop.isBestForSchool && (
              <span className="bg-orange-500 text-white text-[9px] uppercase font-black px-2 py-1 rounded shadow-sm">
                Best for School
              </span>
            )}
          </div>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-tech-blue/10 pointer-events-none group-hover:pointer-events-auto">
            <button
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowQuickView(true); }}
              className="bg-white text-tech-blue px-4 py-2 rounded-xl font-bold text-sm shadow-xl flex items-center gap-2 hover:bg-blue-50 transition active:scale-95"
            >
              <Maximize2 size={16} /> Quick View
            </button>
            <Link to={`/product/${laptop.id}`} className="bg-tech-blue text-white px-4 py-2 rounded-xl font-bold text-sm shadow-xl flex items-center gap-2 hover:bg-blue-900 transition active:scale-95">
        ...
        <div className="p-5 flex flex-col flex-grow">
          <Link to={`/product/${laptop.id}`} className="block mb-2">
            <h3 className="font-black text-gray-900 text-lg leading-tight group-hover:text-tech-blue transition-colors">
              {laptop.name}
            </h3>
          </Link>
          ...
          <div className="mt-auto">
            <div className="flex items-center gap-3 mb-5">
              <span className="text-tech-blue text-2xl font-black">{formatPrice(laptop.price)}</span>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="flex items-center justify-center gap-2 bg-tech-blue text-white py-2.5 rounded-xl text-xs font-black hover:bg-blue-900 transition shadow-lg active:scale-95"
              >
